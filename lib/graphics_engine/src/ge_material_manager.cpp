...
// On native builds `using namespace irr;` is pulled in transitively through
// ge_vulkan_driver.hpp (which only does that inside _IRR_COMPILE_WITH_VULKAN_).
// When Vulkan is disabled (e.g. Emscripten) the using-directive disappears,
// leaving bare `io::IXMLReaderUTF8` references in this file unresolved.
using namespace irr;
